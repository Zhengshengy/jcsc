<template>
    <div class="spotBox bg-white pl-30 float-left mt-20 pb-10">
        <div class="title py-35">
            <div class="shu"></div>
            <span class="text-3 font-18">相关资讯</span>
        </div>
        <div class="item pr-50 w-50 pb-30 float-left pointer" v-for="(item,index) in information" :key="index" @click="toTrends(item.id)">
            ...
            <div class="right float-left ml-20">
                <p class="info font-14 mt-5 singlerow">{{ item.title }}</p>
                <p class="text-6 font-12 mt-10 row2">{{ item.desc }}</p>
            </div>
